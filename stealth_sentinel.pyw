...
# Prevent pythonw.exe from crashing due to missing console stdout/stderr buffering in stealth mode
class DummyWriter:
    def write(self, *args): pass
    def flush(self): pass
if getattr(sys, 'stdout', None) is None: sys.stdout = DummyWriter()
if getattr(sys, 'stderr', None) is None: sys.stderr = DummyWriter()
...
class AnnoyanceWindow(QMainWindow):
    ...
    def show_annoyance(self):
        ...
        if not self.is_showing:
            ...
            rand_bust = random.random()
            url = f"https://www.youtube.com/shorts?random={rand_bust}" if self.url_toggle else f"https://www.youtube.com/feed/trending?random={rand_bust}"
